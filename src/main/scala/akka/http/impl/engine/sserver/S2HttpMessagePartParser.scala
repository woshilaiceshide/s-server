...
abstract class S2HttpMessagePartParser(val settings: spray.can.parsing.ParserSettings,
  ...
  def parseChunk(input: ByteString, offset: Int, closeAfterResponseCompletion: Boolean): Result = {
    ...
    @tailrec def parseSize(cursor: Int = offset, size: Long = 0): Result =
      if (size <= settings.maxChunkSize) {
        byteChar(input, cursor) match {
          case c if spray.can.parsing.CharUtils.isHexDigit(c) ⇒ parseSize(cursor + 1, size * 16 + spray.can.parsing.CharUtils.hexValue(c))
  ...
  def entity(cth: `Content-Type`, body: ByteString): woshilaiceshide.sserver.http.model.HttpEntity = {
    val contentType = if (null == cth) {
      ContentTypes.`application/octet-stream`
    } else {
      cth.contentType
    }
    woshilaiceshide.sserver.http.model.HttpEntity(contentType, HttpData(body.compact))
  }
  ...
  def message(headers: List[HttpHeader], entity: woshilaiceshide.sserver.http.model.HttpEntity): HttpMessagePart
