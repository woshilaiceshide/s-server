 ...
 */
public class RangedInputBuffer implements InputBuffer {
	private final int length;
	private final char[] buffer;

	// the indices of the newline characters in the buffer
	// built lazily, since the newline information is normally only needed in
	// the case of parse errors when
	// error messages need to be generated
	private int[] newlines;

	/**
	 * Constructs a new DefaultInputBuffer wrapping the given char array.
	 * CAUTION: For performance reasons the given char array is not defensively
	 * copied.
	 *
	 * @param buffer
	 *            the chars
	 */
	public RangedInputBuffer(char[] buffer, int length) {
	...
	// returns the zero based input line number the character with the given
	// index is found in
