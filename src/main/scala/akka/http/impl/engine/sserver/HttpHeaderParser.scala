...
import akka.http.impl.engine._
import akka.http.impl.engine.parsing._

final class HttpHeaderParser private (
    val settings: akka.http.impl.engine.parsing.HttpHeaderParser.Settings,
...
private[http] object HttpHeaderParser {
  ...
  def apply(settings: akka.http.impl.engine.parsing.HttpHeaderParser.Settings)(onIllegalHeader: ErrorInfo ⇒ Unit = info ⇒ throw IllegalHeaderException(info)) =
    prime(unprimed(settings, onIllegalHeader))

  def unprimed(settings: akka.http.impl.engine.parsing.HttpHeaderParser.Settings, warnOnIllegalHeader: ErrorInfo ⇒ Unit) =
  ...
  private[http] abstract class HeaderValueParser(val headerName: String, val maxValueCount: Int) {
  ...
  private[http] class ModeledHeaderValueParser(headerName: String, maxHeaderValueLength: Int, maxValueCount: Int, settings: HeaderParser.Settings)
  ...
  private[http] class RawHeaderValueParser(headerName: String, maxHeaderValueLength: Int, maxValueCount: Int)
      extends HeaderValueParser(headerName, maxValueCount) {
    def apply(hhp: HttpHeaderParser, input: ByteString, valueStart: Int, onIllegalHeader: ErrorInfo ⇒ Unit): (HttpHeader, Int) = {
      val (headerValue, endIx) = scanHeaderValue(hhp, input, valueStart, valueStart + maxHeaderValueLength + 2)()
