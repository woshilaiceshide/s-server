...
import akka.http.impl.engine.sserver._
...
object P2SampleHttpServer extends App {
  ...
  val handler = new HttpChannelHandler {
    ...
    private val ping = new woshilaiceshide.sserver.http.model.HttpResponse(200, woshilaiceshide.sserver.http.model.HttpEntity(ContentTypes.`text/plain(UTF-8)`, "Hello World"))
    ...
    private def write_404(channel: HttpChannel) = {
      channel.writeResponse { new woshilaiceshide.sserver.http.model.HttpResponse(400) }
      ResponseAction.responseNormally
    }
    ...
    private def write_ping_asynchronously(channel: HttpChannel) = {
      Future {
        Thread.sleep(3 * 1000);
        channel.writeResponse(new woshilaiceshide.sserver.http.model.HttpResponse(200, "pong0\r\n"))
      }
      ResponseAction.responseNormally
    }
    ...
    private def do_not_support_chunked_request(channel: HttpChannel) = {
      channel.writeResponse { new woshilaiceshide.sserver.http.model.HttpResponse(400, "I DOES NOT support chunked request.") }
      ResponseAction.responseNormally
    }

    def requestReceived(request: woshilaiceshide.sserver.http.model.HttpRequest, channel: HttpChannel, classifier: RequestClassifier): ResponseAction = request match {

      case woshilaiceshide.sserver.http.model.HttpRequest(HttpMethods.GET, uri, _, _, _) if uri.path == path_ping => write_ping(channel)

      case woshilaiceshide.sserver.http.model.HttpRequest(HttpMethods.GET, Uri.Path("/ping_asynchronously"), _, _, _) => write_ping_asynchronously(channel)

      case x @ woshilaiceshide.sserver.http.model.HttpRequest(HttpMethods.GET, Uri.Path("/websocket_demo"), _, _, _) => accept_websocket()

      case x: woshilaiceshide.sserver.http.model.HttpRequest if classifier.classification(x) == RequestClassification.ChunkedHttpStart => do_not_support_chunked_request(channel)

      case _: woshilaiceshide.sserver.http.model.HttpRequest => write_404(channel)
    }
