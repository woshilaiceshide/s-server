...
trait S2ResponseRenderingComponent {
  ...
  def renderResponsePartRenderingContext(
    ...
    def renderResponseStart(response: woshilaiceshide.sserver.http.model.HttpResponse, allowUserContentType: Boolean,
      ...
      def renderEntity(userContentType: Boolean, connHeader: Connection) = {
        response.entity match {
          case woshilaiceshide.sserver.http.model.HttpEntity.NonEmpty(ContentTypes.NoContentType, _) ⇒
          case woshilaiceshide.sserver.http.model.HttpEntity.NonEmpty(contentType, _) if !userContentType ⇒ {
    ...
    def renderResponse(response: woshilaiceshide.sserver.http.model.HttpResponse): Boolean = {
    ...
    def renderChunkedResponseStart(response: woshilaiceshide.sserver.http.model.HttpResponse): CloseMode = {
      val close = renderResponseStart(response,
      ...
      */
      ...
      if (ctx.requestMethod != HttpMethods.HEAD)
        response.entity match {
          case woshilaiceshide.sserver.http.model.HttpEntity.Empty ⇒ // nothing to do
          case woshilaiceshide.sserver.http.model.HttpEntity.NonEmpty(_, data) ⇒ if (chunkless) r ~~ data else r ~~> MessageChunk(data)
        }
    ...
    ctx.responsePart match {
      case x: woshilaiceshide.sserver.http.model.HttpResponse ⇒ CloseMode.closeNowIf(renderResponse(x))
