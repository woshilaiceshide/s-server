...
object HttpConfigurator {
  ...
  def default_akka_parser_settings = akka.http.scaladsl.settings.ParserSettings(com.typesafe.config.ConfigFactory.load())
...
final case class HttpConfigurator(
    ...
    akka_parser_settings: akka.http.scaladsl.settings.ParserSettings = HttpConfigurator.default_akka_parser_settings,
  ...
  private final def borrow_fresh_bytes_rendering(size: Int, response_part: woshilaiceshide.sserver.http.model.HttpResponsePart) = {
    val tmp = new RichByteArrayRendering(size)
    response_part match {
      case response: woshilaiceshide.sserver.http.model.HttpResponse => {
  ...
  private def borrow_bytes_rendering_from_thread_local(size: Int, response_part: woshilaiceshide.sserver.http.model.HttpResponsePart): RichBytesRendering = {
  ...
  private def borrow_bytes_rendering_from_aux_thread(size: Int, response_part: woshilaiceshide.sserver.http.model.HttpResponsePart, aux: AuxThread): RichBytesRendering = {
    if (size <= cached_bytes_rendering_length) {
      response_part match {
        case response: woshilaiceshide.sserver.http.model.HttpResponse => {
          if (response.status eq StatusCodes.OK) {
   ...
   */
  private[http] def borrow_bytes_rendering(size: Int, response_part: woshilaiceshide.sserver.http.model.HttpResponsePart): RichBytesRendering = {
    Thread.currentThread() match {
