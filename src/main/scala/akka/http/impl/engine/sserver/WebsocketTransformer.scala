...
class WebSocketChannel(channel: ChannelWrapper,
  ...
  def tryAccept(request: woshilaiceshide.sserver.http.model.HttpRequest, extraHeaders: List[HttpHeader] = Nil, cookies: List[HttpCookie]): Boolean = {
  ...
  def refuse(response: woshilaiceshide.sserver.http.model.HttpResponse) = {
    val continued = this.synchronized {
      if (this.response_status == ResponseStatus_Init) {
  ...
  private def writeWebSocketResponse(response: woshilaiceshide.sserver.http.model.HttpResponse, writeServerAndDateHeader: Boolean = false) = {
