package woshilaiceshide.sserver.http.model
...
object HttpRequestPart {
  def unapply(part: HttpMessagePart): Option[HttpRequestPart] =
    part.messagePart match {
      case x: HttpRequestPart ⇒ Some(x)
      case _ ⇒ None
    }
}
...
object HttpResponsePart {
  def unapply(part: HttpMessagePart): Option[HttpResponsePart] =
    part.messagePart match {
      case x: HttpResponsePart ⇒ Some(x)
      case _ ⇒ None
    }
}
...
case class HttpRequest(method: HttpMethod = HttpMethods.GET,
  ...
  def withEffectiveUri(securedConnection: Boolean, defaultHostHeader: Host = Host.empty): HttpRequest = {
    ...
    if (hostHeader.isEmpty || uri.authority.isEmpty && hostHeader.get.isEmpty ||
      hostHeader.get.host.equalsIgnoreCase(uri.authority.host) && hostHeader.get.port == uri.authority.port) this
  ...
  def cookies: List[HttpCookiePair] = for (`Cookie`(cookies) ← headers; cookie ← cookies) yield cookie
  ...
  def acceptableContentType(contentTypes: Seq[ContentType]): Option[ContentType] = {
    ...
    @tailrec def findBest(ix: Int = 0, result: ContentType = null, maxQ: Float = 0f): Option[ContentType] =
      if (ix < contentTypes.size) {
        ...
        if (q > maxQ && (ct.charsetOption.isEmpty || isCharsetAccepted(ct.charsetOption.get, charsetRanges))) findBest(ix + 1, ct, q)
        else findBest(ix + 1, result, maxQ)
      } else Option(result)

    findBest() match {
      case x @ Some(ct) if ct.charsetOption.isDefined ⇒ x
      case Some(ct) ⇒
