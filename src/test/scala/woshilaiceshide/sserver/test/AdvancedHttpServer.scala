...
/**
 * A http server with websocket, chunked request, chunked response, plain http and so on.
 *
 * To test it, use `nc -W 60s -C 127.0.0.1 8181 < ./http-requests.dos.txt`
 */
object AdvancedHttpServer extends App {

  val log = org.slf4j.LoggerFactory.getLogger(AdvancedHttpServer.getClass);
  ...
  val handler = new HttpChannelHandler {
    ...
    private def accept_websocket(request: HttpRequest) = ResponseAction.acceptWebsocket {
      websocket(request)
    }
    ...
    private def count_0123456789_with_x_times(channel: HttpChannel, times: Int) = {
      ...
      ResponseAction.acceptChunking(new ChunkedRequestHandler() {
        ...
        def chunkReceived(chunk: MessageChunk): Unit = {
          ...
          log.debug(s"receive from ${channel.remoteAddress}: ${
            bytes.map {
              _.toString
            }.mkString
          }")
    ...
    private def do_not_support_chunked_request(channel: HttpChannel) = {
      println("chunked request is NOT supported")
      channel.writeResponse {
        new HttpResponse(400, "chunked request is NOT supported")
      }
      ResponseAction.responseByMyself
    }
    ...
    def requestReceived(request: HttpRequest, channel: HttpChannel, classifier: RequestClassifier): ResponseAction = request match {
      ...
      case x@HttpRequest(HttpMethods.GET, Uri.Path("/websocket"), _, _, _)
        if classifier.classification(x) == RequestClassification.WebsocketStart => accept_websocket(x)

      case x@HttpRequest(HttpMethods.POST, uri, _, _, _)
        if classifier.classification(x) == RequestClassification.ChunkedHttpStart => {
        log.debug(s"a new request targeted at ${x.uri} from ${channel.remoteAddress}")
        import Uri.Path._
        uri.path match {
      ...
      case x: HttpRequest
        if classifier.classification(x) == RequestClassification.ChunkedHttpStart => do_not_support_chunked_request(channel)
