...
object HttpEntity {
  ...
  def apply(contentType: ContentType, string: String): HttpEntity =
    if (string.isEmpty) Empty else apply(contentType, contentType.charsetOption match {
      case None => HttpData(string)
      case Some(cs) => HttpData(string, cs)
    })
  ...
  case class NonEmpty private[HttpEntity] (contentType: ContentType, data: HttpData.NonEmpty) extends HttpEntity {
    ...
    def asString = data.asString(contentType.charsetOption.getOrElse(HttpCharsets.`UTF-8`))
    def asString(defaultCharset: HttpCharset) = data.asString(contentType.charsetOption getOrElse defaultCharset)
