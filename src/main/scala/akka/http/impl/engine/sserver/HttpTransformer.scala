...
object HttpTransformer {
  ...
  private[HttpTransformer] final case class Node(value: woshilaiceshide.sserver.http.model.HttpRequestPart, closeAfterResponseCompletion: Boolean, channelWrapper: ChannelWrapper, var next: Node)
...
class HttpTransformer(handler: HttpChannelHandler, configurator: HttpConfigurator)
  ...
  @inline private def en_queue(request: woshilaiceshide.sserver.http.model.HttpRequestPart, closeAfterResponseCompletion: Boolean, channelWrapper: ChannelWrapper) = {
    if (null == head) {
  ...
  def bytesReceived(byteBuffer: java.nio.ByteBuffer, channelWrapper: ChannelWrapper): ChannelHandler = {
    ...
    @scala.annotation.tailrec def process(result: Result): ChannelHandler = {
      ...
      result match {
        ...
        case r: Result.AbstractEmit /*if (r.part.isInstanceOf[woshilaiceshide.sserver.http.model.HttpRequestPart])*/ /*it's woshilaiceshide.sserver.http.model.HttpRequestPart definitely.*/ => {

          import r._
          val request = r.part.asInstanceOf[woshilaiceshide.sserver.http.model.HttpRequestPart]

          request match {
            case x: woshilaiceshide.sserver.http.model.HttpRequest => {
